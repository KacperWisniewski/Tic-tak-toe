...
export default {
    ...
    methods:{
        ...
        changeSign(box){
            ...
            for(let i=0; i<3; i++) {
                if (this.boxes[(i*3)].sign === this.boxes[((i*3)+1)].sign && this.boxes[(i*3)].sign === this.boxes[((i*3)+2)].sign && this.boxes[i*3].sign !== 0) {
                    this.playerWin = this.boxes[(i*3)].sign
                    break
                }
            }
            for(let i=0; i<3; i++){
                if (this.boxes[i].sign === this.boxes[(i+3)].sign && this.boxes[i].sign === this.boxes[i+6].sign && this.boxes[i] !== 0) {
                    this.playerWin = this.boxes[i].sign
                    break
                }
            }
            if (this.boxes[0].sign === this.boxes[4].sign && this.boxes[0].sign === this.boxes[8].sign) {
